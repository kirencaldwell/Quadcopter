...
df = pd.read_csv("bazel-out/k8-fastbuild/bin/quadcopter_test.runfiles/Quadcopter/quadcopter_test.csv")
...
fig, ax = plt.subplots(3)
ax[0].plot(df.t, df.A0, linewidth=4)
ax[0].grid(True)
ax[1].plot(df.t, df.A1, linewidth=4)
ax[1].grid(True)
ax[2].plot(df.t, df.A2, linewidth=4)
ax[2].grid(True)
plt.show()
...
ax[2].grid(True)
ax[3].plot(df.t, df.f0, linewidth=4)
ax[3].grid(True)
plt.show()